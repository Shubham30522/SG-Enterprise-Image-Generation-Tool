...
2.  **Create Directories & Color Setup**
    ...
    - **Determine Color Folder**:
        ...
        - If **NO** color is specified, you must **STOP AND ASK** the user: *"Please specify the color name for the input folder (e.g., 'Grey', 'Navy', '01-Black')."*
...
3.  **Smart Angle Selection (Logical Analysis)**
    ...
    - **If Reference Images ARE Provided**:
        - Create 5 prompt files corresponding *exactly* to the angles/layouts in the provided reference images.
...
4.  **Populate Content & Fidelity**
    - **Step 4a (Adaptation)**: Populate these files using templates from `configure_prompts.py` as a base, but **REWRITE** the content to match your selected angles.
        - Example: If you chose `waistband.txt`, do NOT paste the "Neck" template blindly. Rewrite it: *"Close-up detail of the waistband, button, and zipper fly."*
    - **Step 4b (Style Consistency)**:
        - Inherit the *Style* (Lighting, Background) from existing products (e.g., "T-Shirt for Women") if no reference is given.
